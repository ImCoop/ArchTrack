...
export const notificationService = {
  list(userId: string, filters?: { readStatus?: 'read' | 'unread' }) {
  ...
  async notifyRole(
    role: Role,
    input: Omit<Notification, 'id' | 'readAt' | 'createdAt' | 'userId'>,
  ) {
  ...
  updatePreferences(userId: string, input: Partial<Omit<NotificationPreferences, 'userId'>>) {
