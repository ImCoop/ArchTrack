...
` + $[Z].replace(" at new ", " at ");
    ...
    function ft(d) {
      if (d) {
        var w = d._owner, v = lt(d.type, d._source, w ? w.type : null);
        In.setExtraStackFrame(v);
      } else
        In.setExtraStackFrame(null);
    }
    function Ss(d, w, v, I, W) {
      {
    ...
    function Ps(d, w) {
      {
        var v = function() {
          jn || (jn = !0, T("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", w));
        };
    ...
    function Rs(d, w) {
      {
        var v = function() {
          $n || ($n = !0, T("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", w));
        };
    ...
    function js(d, w, v, I, W) {
      {
        var V, D = {}, $ = null, ae = null;
        v !== void 0 && (Rn(v), $ = "" + v), Cs(w) && (Rn(w.key), $ = "" + w.key), ks(w) && (ae = w.ref, Is(w, W));
        for (V in w)
          Ge.call(w, V) && !Es.hasOwnProperty(V) && (D[V] = w[V]);
        if (d && d.defaultProps) {
    ...
    function Kn(d, w, v, I, W, V) {
      {
        ...
        if (Ge.call(w, "key")) {
          var Ce = q(d), ue = Object.keys(w).filter(function(Ws) {
            return Ws !== "key";
...
function wn(n) {
  ...
  if (s.revoke.push(u), jr.add(c), a.proxy = c, t) {
    ...
    l.finalities.draft.push((f, h) => {
      ...
      if (_) {
        let T = _.original;
        _.operated && (T = gn(S)), Yt(_), Zt(_, it, f, h), l.options.enableAutoFreeze && (l.options.updatedValues = (y = l.options.updatedValues) !== null && y !== void 0 ? y : /* @__PURE__ */ new WeakMap(), l.options.updatedValues.set(T, _.original)), ot(m, r, T);
      }
...
const ai = (n) => function e(t, r, s) {
  ...
  const R = (O) => {
    ...
    if (!$e(O)) {
      ...
      if (C) {
        const G = C[0];
        return m.strict && typeof O == "object" && O !== null && en({
          rootDraft: F,
          value: O,
          useRawReturn: !0
        }), _([G]);
...
function nn(n, e, t) {
  ...
  if (a) {
    for (const f of a.keys()) {
      ...
      h && h["on-delete-reverse"] === "cascade" && fe(a.get(f), 1).forEach(([y, b, g]) => {
        var m;
        return nn(n, e, [g, (m = h["reverse-identity"]) == null ? void 0 : m[1]]);
  ...
  c && c.forEach((f) => {
    ...
    (!s || !g || ((m = g["reverse-identity"]) == null ? void 0 : m[1]) === s) && (_e(n.eav, [h, y, b]), _e(n.aev, [y, h, b]), _e(n.vae, [b, y, h])), g && g["on-delete"] === "cascade" && ((S = g["reverse-identity"]) == null ? void 0 : S[1]) === s && nn(n, e, [h, (_ = g["forward-identity"]) == null ? void 0 : _[1]]);
...
function ha({ attrsStore: n, schema: e }, t) {
  ...
  function o(b, g) {
    return re(n, b, g) || s.find((m) => m["forward-identity"][1] === b && m["forward-identity"][2] === g);
  }
  function a(b, g) {
  ...
  function f(b, g) {
    ...
    u(m), u(S), !m && !S && c(sr(e, b, g, fa));
  }
  for (const b of t)
    for (const { etype: g, lookupPair: m, linkLabel: S } of da(b)) {
      const _ = m[0];
      if (S) {
        f(g, S);
  ...
  for (const b of t) {
    ...
    if (aa.has(g)) {
      const T = o(m, "id");
      u(T), T || c(pt(e, m, "id", { "unique?": !0 }));
      for (const R of Object.keys(_)) {
        const O = o(m, R);
        if (u(O), ua.has(g) && (O || c(pt(e, m, R, R === "id" ? { "unique?": !0 } : null))), ca.has(g)) {
          const F = a(m, R);
          !O && !F && c(sr(e, m, R)), u(F);
        }
      }
      ...
      e: for (const [m, S] of Object.entries(u))
        for (let _ = 0; _ < f.length; _++) {
          ...
          if (Ci(T.store, m)) {
            ...
            const R = qt(s, T.store, l.attrsStore), O = Ha(T.store, l.attrsStore, S)[m];
            R ? (h.push({
              oldEntity: T.entity,
              newEntity: R,
              changedFields: O || {}
...
class yc {
    ...
    switch (wr[t.op] || this._log.info("[receive]", e, t.op, t), t.op) {
      ...
      case "add-query-ok": {
        ...
        if (!this._hasQueryListeners() && !this.querySubs.currentValue[g])
          break;
        const m = (c = (a = b == null ? void 0 : b[0]) == null ? void 0 : a.data) == null ? void 0 : c["page-info"], S = (l = (u = b == null ? void 0 : b[0]) == null ? void 0 : u.data) == null ? void 0 : l.aggregate, _ = fr(b), T = this.ensureAttrs(), R = et(T, _, r, this.config.useDateObjects);
      ...
      case "refresh-ok": {
        ...
        const S = Vt(m.entries()), _ = y.map((T) => {
          var q, E, j, M;
  ...
  _handleReceiveError(e) {
    ...
    if ((u = e["original-event"]) != null && u.hasOwnProperty("q") && ((l = e["original-event"]) == null ? void 0 : l.op) === "add-query") {
      const g = (f = e["original-event"]) == null ? void 0 : f.q, m = ne(g);
      this.notifyQueryError(ne(g), s), this.notifyQueryOnceError(g, m, t, s);
      return;
    }
